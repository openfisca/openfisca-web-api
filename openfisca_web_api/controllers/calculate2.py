...
def api2_calculate(req):
        ...
        for scenario_index, scenario in enumerate(scenarios):
            ...
            for variable_name in variables:
                try:
                    ...
                    raise wsgihelpers.respond_error_json(
                        ctx,
                        code = 500,
                        details = exc.to_json(),
                        headers = headers,
                        inputs = inputs,
                        message = ctx._(u'Legislation parameter not found'),
    ...
    if content_type != expected_content_type:
        return wsgihelpers.respond_error_json(
            ctx,
            code = 400,
            headers = headers,
            hint = ctx._(u'Content-Type must be explicitely set to "{}" in your HTTP request. '
            u'See http://doc.openfisca.fr/hints/web-api-http-request-bad-content-type'.format(
    ...
    if error is not None:
        return wsgihelpers.respond_error_json(
            ...
            code = 400,
            headers = headers,
            hint = ctx._(u'The JSON payload in the body of your HTTP request must be valid. '
            u'See http://doc.openfisca.fr/hints/web-api-http-request-invalid-json'),
            message = ctx._(u'JSON in request body: {}'.format(error)),
    ...
    if errors is not None:
        return wsgihelpers.respond_error_json(
            ctx,
            code = 400,
            details = conv.jsonify_value(errors),  # Because some dict keys are not strings.
            headers = headers,
            inputs = inputs,
            message = ctx._(u'Bad parameters in request JSON payload'),
            type = 'INVALID_JSON_PAYLOAD',
            )
